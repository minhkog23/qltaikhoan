            ...
            <div class="tile-body">
              <div class="row element-button">
                <div class="col-sm-2">
                  ...
                  <!-- <a class="btn btn-add btn-sm" href="form_add_DanhMuc.php" title="Thêm"><i class="fas fa-plus"></i>
                    Tạo mới danh mục</a> -->
                </div>
                
              </div>
              <?php
                $p->getUser("SELECT * FROM users order by id_user desc");
                // xóa
                if(isset($_POST['nut']) && $_POST['nut']=="Xóa")
                {
                  $id_xoa=$_POST['id_xoa'];
                  if($p->themxoasua("DELETE FROM loaitaikhoan WHERE id_maTK=$id_xoa")==1)
                  {
                    echo '<script>
